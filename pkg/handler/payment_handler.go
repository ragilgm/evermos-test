...
type PaymentHandler struct {
	oUcase usecase.TransactionUseCase
}

func NewPaymentHandler(e *echo.Echo, us usecase.TransactionUseCase) {
	handler := &PaymentHandler{
		oUcase: us,
	}
	// ENDPOINT
	e.POST("/transactions/payments", handler.PaymentProcess)
}
...
func (p PaymentHandler) PaymentProcess(context echo.Context) (err error) {
	...
	// call useCase
	res, err := p.oUcase.Process(&request)
